...
class MyPlayer(PlayerHex):
    ...
    def _get_shortest_path_distance(self, state: GameStateHex, piece_type: str) -> float:
        ...
        Includes Virtual Connections (Bridges).
        """
        board = state.get_rep()
        env = board.get_env()
        ...
        # (offset_r, offset_c), ((shared1_r, shared1_c), (shared2_r, shared2_c))
        bridge_data = [
            ((-1, 2), ((0, 1), (-1, 1))),
            ((1, 1), ((0, 1), (1, 0))),
            ((2, -1), ((1, 0), (1, -1))),
        ...
        while pq:
            ...
            # 2. Virtual Connections (Bridges)
            for (dr, dc), shared in bridge_data:
                nr, nc = r + dr, c + dc
                if 0 <= nr < rows and 0 <= nc < cols:
                    # Check if bridge is intact (both shared neighbors are empty)
                    s1r, s1c = r + shared[0][0], c + shared[0][1]
                    s2r, s2c = r + shared[1][0], c + shared[1][1]
                    
                    # Shared neighbors must be in board and empty
